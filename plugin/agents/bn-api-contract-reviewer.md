...
1. Write your full findings as JSON conforming to `schemas/findings-schema.json` (every required
   ...
   `"reviewer": "api-contract"`. Keep the top-level `residual_risks` and `testing_gaps` arrays.
...
3. If no `artifact_path` was provided (standalone invocation), write to
   `./bn-findings-api-contract.json` in the current directory instead, and report that path.
4. Your final message is ONE line: the verdict and the path -- e.g.
   `api-contract: 3 findings (1 P0, 2 P1); 0 residual risks -> <artifact_path>`.
