---
name: bn-best-practices-researcher
...
2. **Identify Relevant Skills**:
   Match the research topic to available skills by reading their descriptions. Look for skills whose
   stated domain (a language, framework, design discipline, architecture style, or tooling area)
   overlaps the topic, and prefer skills that name the exact technology being researched over those
   that only touch it tangentially.
...
2. If no `artifact_path` was provided (standalone invocation), write to `./bn-brief-best-practices.md`
