...
- `bash ${CLAUDE_PLUGIN_ROOT}/skills/bn-resolve-pr/scripts/get-pr-comments` -- GraphQL query for unresolved review threads
- `bash ${CLAUDE_PLUGIN_ROOT}/skills/bn-resolve-pr/scripts/get-thread-for-comment` -- Map a comment node ID to its parent thread (for targeted mode)
- `bash ${CLAUDE_PLUGIN_ROOT}/skills/bn-resolve-pr/scripts/reply-to-pr-thread` -- GraphQL mutation to reply within a review thread
- `bash ${CLAUDE_PLUGIN_ROOT}/skills/bn-resolve-pr/scripts/resolve-pr-thread` -- GraphQL mutation to resolve a thread by ID
