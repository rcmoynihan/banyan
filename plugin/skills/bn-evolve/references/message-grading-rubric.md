...
`${CLAUDE_PLUGIN_ROOT}/skills/bn-conventions/references/envelope.md`). Each axis lists its canonical
...
- `artifacts-over-prose` — Targets the invariant-3 return contract
  (`${CLAUDE_PLUGIN_ROOT}/AGENTS.md`, invariant 3): the brief is a verdict-plus-path, never the
...
`${CLAUDE_PLUGIN_ROOT}/skills/bn-conventions/references/process-pitfalls.md`. This rubric's only
