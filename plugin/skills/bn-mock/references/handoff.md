# Routing discipline — propose-never-patch handoff + disposition table (R18–R21)

This is the `/bn-mock` handoff: it routes the mock-notes' findings forward through the **owning
skills** so the learning becomes durable (R18), **without ever editing a protected artifact**.
`SKILL.md` invokes this rule after reading the mock-notes gate artifact.

## The identity rule: propose, never patch (R18 / R19 / AGENTS.md §5)
...
- **fold into requirements** ⇒ dispatch `/bn-brainstorm <doc>` in the current session, passing the
  ...
  requirements doc — writes the change (R19). The trunk never edits the doc itself.
  - **AE5 (free-text input, no doc exists):** there is no requirements doc to fold into. Instead
...
- **send to spec-stress** ⇒ **SYNTHESIZE a finalized requirements summary** from the mock-notes'
  *planning-impact* + *suggested-patches* sections and pass that summary to `/bn-spec-stress`
  (R19). Never pass a raw `briefs/mock-notes.md` path — `/bn-spec-stress` accepts a requirements
...
## Plan-impact second axis (R21) — only when the input was a plan path
...
### AE3 walkthrough (plan input, known-wrong plan)
...
> that finding `replan`, and — because the input was a plan path — the R21 axis marks it
...
R21 axis alongside each finding. The notes path is named as the durable record; the *routing* is
what turns it into durable knowledge (R18).
