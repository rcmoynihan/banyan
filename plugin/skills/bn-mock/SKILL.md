...
advisory.** The slug is interpolated into `mock/<slug>/` for every builder Write target and into
the `rm -rf mock/<slug>/` cleanup command, so a slug containing `/`, `..`, a leading `/`, or an
empty string would let a Write escape the mock tree (e.g. `mock/../../plugin/...`) or collapse the
cleanup to `rm -rf mock/`. If the derived slug does **not** match, re-derive it; if it still cannot
be made to match, STOP — do **not** populate the envelope's `inputs.slug` and do **not** spawn the
builder. Only a regex-validated slug ever reaches the envelope or the cleanup line. Do not pre-read
full doc/plan content into trunk context — verify only enough to classify and pass the path to the
builder.
...
- manifest present AND `slug` + `source_kind`/`source_input` all match ⇒ in-place iteration,
  proceed. **Compute the next iteration number here** (`N+1` from the manifest's current
  `iteration`) and pass it as `inputs.iteration` in Step 4 — the trunk owns the increment; the
  builder writes the value verbatim and appends `## Iteration N`.
...
envelope and the Step 5 read-back:

- If `.banyan/runs/<run-id>/briefs/mock-notes.md` does **not** exist, or it exists and is for **this
  same slug** (an in-place iteration — its `# Mock notes: <slug>` heading matches), use the
  unsuffixed `.banyan/runs/<run-id>/briefs/mock-notes.md`.
- If it exists for a **different** slug, use the slug-suffixed
  `.banyan/runs/<run-id>/briefs/mock-notes-<slug>.md` instead, so this idea's notes never overwrite
  the prior idea's.
...
- **Cleanup:** surface the exact `mock/<slug>/` path and the cleanup command
  ...
  regex-validated slug from Step 1 into this `rm -rf` line — the Step 1 gate guarantees the slug is
  non-empty and free of `/`, `..`, or a leading `/`, so the command is safe by construction; never
  print an `rm -rf` line built from an unvalidated slug (an empty slug would collapse to
  `rm -rf mock/`, deleting every mock; `..` would escape the mock tree). Never delete `mock/`
  yourself and never instruct the builder to.
- **.gitignore:** OFFER (via `AskUserQuestion`) to add `mock/` to `.gitignore`, never
