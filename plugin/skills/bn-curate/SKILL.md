...
- **blank or a run-id** → `consolidate` (the default): dedup staged candidates, merge/promote
  (causal claims gated on `claim_type: tested`), mark stale, minimal discoverability. This is the
  only mode dispatched in the **background**. Steps 1-4 below build this path.
- **`--refresh [scope]`** → `refresh`: reconcile the existing `docs/solutions/` store against the
...
- **`--concepts`** → `concepts`: bootstrap `CONCEPTS.md` from the repo's declared domain model.
  **Foreground only** (see Step 6). The only path that *creates* `CONCEPTS.md`; normal
  consolidation only updates an existing index.
...
boundaries:      WRITE SCOPE (pre-granted) is ONLY: docs/solutions/ (promote/merge),
                 ...
                 + this summary artifact. EVERYTHING ELSE IS REPORT-ONLY -- never edit
                 source, config, tests, docs/plans, docs/brainstorms, or other docs/runs
                 ...
                 frontmatter. NEVER delete a docs/solutions/ doc in this mode (or any
...
consolidate. Refresh reconciles the existing `docs/solutions/` store against the current codebase
...
docs/solutions/ against the codebase: keep / update-in-place / consolidate-superseded /
mark-stale (body prose) / report". Pick the summary artifact path under the most recent run (or a
path the user names). If `docs/solutions/` is empty or the scope matches nothing, the curator
...
Deletion of a protected `docs/solutions/` doc is permitted only when all three hold: a
...
*creates* `CONCEPTS.md`; normal consolidation only updates an existing index, and never
auto-creates one in the background. It is **foreground only**.
...
the write scope is **pre-granted in the envelope** (docs/solutions/, CONCEPTS.md, CLAUDE.md, and
clearing the named lessons-staging/), and **anything outside that scope is report-only**: the
...
foreground, per-doc user-confirmed), and concepts-mode creation of `CONCEPTS.md` is an explicit
