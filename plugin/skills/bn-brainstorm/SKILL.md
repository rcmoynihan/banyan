...
**Greenfield-product trajectory — ask, never assume.** When the brainstorm describes a *new standalone product the user is inventing* — a greenfield idea **not** framed as feature work inside an existing product, company, or organization — the idea's intended reach is a product-shaping fact you must not infer. Before classifying feature vs product and before Phase 1.2, ask one early `AskUserQuestion` to ground trajectory, unless the opening already states it. Offer the spectrum as first-class, mutually valid options — a personal/local-only tool is a legitimate destination, not a lesser one; do not nudge toward scale:

- **Personal / local-only** — just the user, on their own machine.
- **Internal / small team** — a handful of known people in one group.
- **Multi-user product** — others adopt it; reach matters, no commercial intent stated.
- **Commercial product** — intended to go to market and capture value (monetization, GTM).

(The tool's free-text fallback covers anything off this spectrum.) This question does **not** fire for feature work inside an existing product, company, or org (that brainstorm inherits the host product's trajectory), for clearly-bounded Lightweight work, or when the opening already pins the trajectory.

The answer sets product-vs-feature and downstream rigor. **Multi-user** or **commercial** make the brainstorm product-tier — run the Deep — product lenses and sections even when other product-shape signals were quiet — and arm the gated reach/capture probe in Phase 1.2. **Personal/local** or **team** keep the brainstorm at feature tier and suppress the reach/capture probe. Record the chosen trajectory as a Key Decision (or as an Assumption if the user was unsure) so planning inherits it.
...
- **Reach & capture gap** *(fires only when the Phase 0.3 trajectory answer is multi-user or commercial)*. The opening assumes adoption or revenue without naming who actually adopts at scale or how the product reaches them and — for a commercial trajectory — what someone pays for and why now. When present, probe who specifically adopts this beyond the user, how they find it, and (if commercial) how it captures value. Keep this at grounding depth — enough to shape scope, success criteria, and Scope Boundaries — not a full GTM plan. Record genuine uncertainty as Assumptions or Outstanding Questions rather than inventing a business model.
...
1. **Open a run lazily** if this branch has not already. Reuse the grounding branch's run if one is open (add a coverage unit to its ledger); in `/bn-grow` intake reuse the caller's ledger. Otherwise:
   `node ${CLAUDE_PLUGIN_ROOT}/skills/bn-conventions/scripts/new-run.mjs brainstorm-<slug> --root <repo-root> --objective "<the brainstorm question>" --plan-ref "none -- brainstorm convergence gate" --unit "coverage|bn-brainstorm-coverage-reviewer|in-progress|.banyan/runs/<run-id>/briefs/brainstorm-coverage.md" --actor trunk`
