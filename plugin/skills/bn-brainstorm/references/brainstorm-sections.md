...
- **The planning agent** (`bn-plan` or a human) produces an implementation
...
- **Success Criteria** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("the planner can act on this
...
downstream tooling depends on. The fields appear as YAML frontmatter at the
top of the file.
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to bn-plan, no italic provenance lines. Engineering process
...
`references/markdown-rendering.md` describes how to render these sections on
the page. This reference (`brainstorm-sections.md`) is about WHAT the
brainstorm contains; the rendering reference is about HOW it is presented.
