...
This file is loaded when bn-brainstorm detects a non-software task (Phase 0). It replaces the software-specific brainstorming phases (Phases 0.2 through 4) with facilitation principles for any domain. The Core Principles and **Interaction Rules** in the parent `bn-brainstorm/SKILL.md` still apply unchanged — including one-question-per-turn and the default to the platform's blocking question tool. This file extends those rules with universal-domain facilitation guidance; it does not relax them.
...
**Then offer next steps** using `AskUserQuestion`. Never silently skip the question.

**Question:** "Brainstorm wrapped. What would you like to do next?"

- **Create a plan** → hand off to `/bn-plan` with the decided goal and constraints
