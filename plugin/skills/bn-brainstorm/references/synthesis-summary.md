...
- **Naming implementation detail in any bullet**: file paths, module names, exact JSON keys, HTTP status codes, error message wording, SQL syntax. The synthesis is scope-only; implementation is bn-plan's job. These granularity rules apply to every bullet in every section.
...
- "Use Postgres for persistence" — implementation detail (bn-plan's job), failed granularity rules
...
If the user response indicates they're in the wrong skill or want a different workflow (e.g., "this is too small, just /bn-work it" or "this needs more thought, let me brainstorm differently"):

- Stop bn-brainstorm
- Suggest the alternative skill the user appears to want (e.g., `/bn-work`, `/bn-debug`)
