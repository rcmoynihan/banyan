...
function hasStock(store, sku, qty) {
  // BUG-08 (performance): deep-clones the ENTIRE inventory map on every stock
  // check before reading a single product. On a large catalog this turns an
  // O(1) lookup into an O(n) copy, and hasStock is called in hot loops.
  const snapshot = new Map();
  for (const [key, value] of store.inventory) {
    snapshot.set(key, JSON.parse(JSON.stringify(value)));
  }
  const product = snapshot.get(sku);
...
function reserve(store, sku, qty) {
  ...
  // BUG-04 (off-by-one): uses `<` where it should use a check that also fails
  // when qty exceeds stock by one. With strict `<`, asking for stock+1 passes
  // the guard and drives stock negative (oversell).
