...
// R2-1: the copied skill assets (scripts/ + references/) are guarded by the gate,
// not just the SKILL.md. A hand-edit of a copied script must red the gate so a
// drifted runtime asset cannot ship silently.
test('a hand-edit of a copied skill asset is reported as drift', () => {
  const root = freshRoot();
  try {
    ...
    fs.appendFileSync(target, '\n// hand-edit no source produces\n');
    const drift = detectDrift(root);
    assert.ok(
      drift.some(
        (d) => d.kind === 'modified' && d.path === 'skills/bn-conventions/scripts/new-run.mjs',
      ),
      `expected a modified copied-asset drift entry, got:\n${formatDrift(drift)}`,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
// R2-1: a missing copied asset must red the gate, so a render that dropped a
// runtime-referenced script or reference cannot pass.
test('a missing copied skill asset is reported as drift', () => {
  const root = freshRoot();
  try {
    fs.rmSync(
      path.join(root, 'dist', 'codex', 'skills', 'bn-conventions', 'references', 'envelope.md'),
    );
    const drift = detectDrift(root);
    assert.ok(
      drift.some(
        (d) =>
          d.kind === 'missing' && d.path === 'skills/bn-conventions/references/envelope.md',
      ),
      `expected a missing copied-asset drift entry, got:\n${formatDrift(drift)}`,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
