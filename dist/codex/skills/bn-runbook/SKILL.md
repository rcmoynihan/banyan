...
- `~/.codex/skills/banyan/AGENTS.md` (the eight invariants; §5 protected artifacts; §6 the
  permission cliff)
...
- `~/.codex/skills/banyan/skills/bn-runbook/references/tiering.md` — the five-tier decision rule
  and the read-only-on-source contract (the forbidden-Bash list).
- `~/.codex/skills/banyan/skills/bn-runbook/references/execute-validate.md` — the budget-bounded
  proven-vs-declared pass.
- `~/.codex/skills/banyan/skills/bn-runbook/references/plan-handoff.md` — the no-drivable-path /
  cheap-fruit requirements-doc route into `/bn-plan`.
- `~/.codex/skills/banyan/skills/bn-runbook/references/recipe-write.md` — the approval-gated write,
  re-run safety, and the `last_validated` marker.

The recipe shape is fixed by the shared contract, never hand-rolled here:

- schema: `~/.codex/skills/banyan/schemas/drive-recipe.schema.json`
- validator/extractor/reconcile + CLI:
  `~/.codex/skills/banyan/skills/bn-conventions/scripts/validate-drive-recipe.mjs`
...
3. Open or reuse the run ledger:
   ...
   ```bash
   node ~/.codex/skills/banyan/skills/bn-conventions/scripts/new-run.mjs runbook-<repo-slug> --root <repo-root>
   ```
...
```bash
node ~/.codex/skills/banyan/skills/bn-conventions/scripts/validate-drive-recipe.mjs <candidate-file>
```
