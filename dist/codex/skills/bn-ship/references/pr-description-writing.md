...
- Bad: "Adds `evidence-decider.ts`, modifies `SKILL.md` to call it, and updates two test files."
...
In order: opening → body sections that earn their keep → test plan if non-obvious → evidence block if one exists → optional badge after a `---` rule.
...
**Evidence handling:** preserve any existing `## Demo` or `## Screenshots` block verbatim unless the user's focus asks to refresh it. If the user supplied a URL or markdown embed, splice as `## Demo`. Otherwise omit. Place before the badge. Never label test output as "Demo" or "Screenshots."
...
## Step D: Badge (optional)

Optionally append a harness/model badge after a `---` rule if the repo's PRs carry one; otherwise omit. If a badge embeds a model slug inside a markdown image URL, **URL-encode literal parens as `%28` / `%29`** — unencoded parens break release-please's commit parser, which silently drops the commit from the changelog.
